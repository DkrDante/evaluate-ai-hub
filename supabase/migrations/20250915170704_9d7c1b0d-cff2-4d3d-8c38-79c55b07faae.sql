...
-- Create evaluation_jobs table to store evaluation runs
...
-- Create policies for evaluation_jobs
...
CREATE POLICY "Users can update their own evaluation jobs" 
ON public.evaluation_jobs 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own evaluation jobs" 
ON public.evaluation_jobs 
FOR DELETE 
USING (auth.uid() = user_id);
